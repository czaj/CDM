function ResultsOut = genOutput_CDM(EstimOpt,Results,Head,Tail,Names,Template1,Template2,Heads,ST,Type,Statistics)
...
head_nb3 = {'var.', '','coef.','' ,'st.err.' , ''};
head_nb2 = {'coef.','' ,'st.err.' , ''};
Coords = struct;
...
for i = 1:Dim1
    ...
    if i ~= Dim1
        ...
        for s=1:size(Block,2)/4
            if EstimOpt.NB==1 && strcmp({'DetailsNB'},subsref(fieldnames(Heads), struct('type', '()', 'subs', {{i+1, 1}}))) % use subsref since indexing before saving does not work 
                RowOut(:,4*s-2) = cellstr('');
                RowOut(:,4*s) = cellstr('');
            else
        ...
        if fixed == 0 
            ...
            if size(Block,2)/4 >1
                ...
                if EstimOpt.NB==1 && strcmp({'DetailsNB'},subsref(fieldnames(Heads), struct('type', '()', 'subs', {{i+1, 1}}))) % use subsref since indexing before saving does not work 
                    headn1 = [head_nb3, repmat(head_nb2, 1, size(Block,2)/4 - 1)];
                end
            else
                headn1 = head3;
                if EstimOpt.NB==1 && strcmp({'DetailsNB'},subsref(fieldnames(Heads), struct('type', '()', 'subs', {{i+1, 1}}))) % use subsref since indexing before saving does not work 
        ...
        %generating headlines within code
        for s=1:headssize
...
%     [~,CW] = CellColumnWidth(ResultsOut(FirstBlock:FirstBlock+size(Results.(Template2{i,1}),1)-1,:));
    ...
    disp(' ')
    cprintf('*Black', 'Descriptive Statistics \n')

    if strlength(char(EstimOpt.NamesA(EstimOpt.TC_par_index))) >17
        TC_par_name = strcat(extractBefore(char(EstimOpt.NamesA(EstimOpt.TC_par_index)),17),'...');
        padding_TC_par = 0;
    else
